...
xgboostpath = "path/to/your/xgboost/"
...
results_file_path = os.path.join(xgboostpath, "xgboost_ablation_study_results_v1.csv")
best_config_file_path = os.path.join(xgboostpath, "xgboost_ablation_best_config_v1.csv")
fig_outfile_path = = os.path.join(xgboostpath, "xgboost_ablation_test_hyperpara_heatmap.pdf")
...
xgboost_params = [
    {"n_estimators": ne, "max_depth": md, "learning_rate": lr, "gamma": g, "alpha": a, "colsample_bytree": cbt}
    for ne in [50,100]
    for md in [6,7,8]
    for lr in [0.0000001,0.000001,0.00001,0.0001]
    for g in [0, 1]
    for a in [1.5,2,2.5,3]
    for cbt in [0.15,0.2,0.25]
]


ablation_params = {"XGBoost": xgboost_params}
...
roc_curve_data = {}
threshold = 1.3  # Set your threshold for the composite score
